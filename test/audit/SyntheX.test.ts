...
describe("SyntheX", function () {
  ...
  before(async function () {
    ...
    token = await MockToken.deploy("Test", "TST", 18);

    const SyntheXToken = await ethers.getContractFactory("SyntheXToken");
    syx = await SyntheXToken.deploy(syntheX.address);
    ...
    esSYX = (await upgrades.deployProxy(EscrowedSYX, [
      syntheX.address,
      syx.address,
      token.address,
  ...
  describe("Pause/unpause", function () {
    ...
    it("Shouldn't pause and unpause contract by everyone but L2Admin", async () => {
      await expect(syntheX.connect(alice).pause()).to.be.revertedWith("15");
      await expect(syntheX.connect(charlie).pause()).to.be.revertedWith("15");
    });
    ...
    it("Shouldn't call specified any function when paused", async () => {
      ...
      await expect(
        syntheX
          .connect(bob)
          ["distribute(address,uint256,uint256)"](token.address, 100, 100)
      ).to.be.revertedWith("Pausable: paused");
      await expect(
        syntheX.connect(bob)["distribute(uint256)"](100)
      ).to.be.revertedWith("Pausable: paused");
    });
  });
  ...
  describe("Rewards operations", function () {
    it("Should set pool speed with adding reward token to list", async () => {
      ...
      await expect(
        syntheX
          .connect(bob)
          .setPoolSpeed(token.address, pool.address, speed, addToList)
      )
        .to.emit(syntheX, "SetPoolRewardSpeed")
        .withArgs(token.address, pool.address, speed);

      expect(await syntheX.rewardSpeeds(token.address, pool.address)).to.equal(
        speed
      );
      expect(await syntheX.rewardTokens(pool.address, 0)).to.equal(
        token.address
      );
    });
    ...
    it("Should set pool speed without adding reward token to list", async () => {
      ...
      await syntheX
        .connect(bob)
        .setPoolSpeed(token.address, pool.address, speed2, !addToList);
    ...
    it("Should update pool reward index", async () => {
      ...
      const poolRewardStateBefore = await syntheX.rewardState(
        token.address,
        pool.address
      );
      ...
      await syntheX.connect(poolSigner)["distribute(uint256)"](totalSupply);
      const poolRewardState = await syntheX.rewardState(
        token.address,
        pool.address
      );
    ...
    it("Should claim rewards", async () => {
      ...
      await syx
        .connect(alice)
        .mint(owner.address, ethers.utils.parseEther("100000000"));
      await syx.increaseAllowance(
        esSYX.address,
      ...
      await syntheX
        .connect(bob)
        .setPoolSpeed(esSYX.address, pool.address, speed, addToList);
      ...
      const synth = await upgrades.deployProxy(
        ERC20X,
        ["Ethereum", "ETH", pool.address, syntheX.address],
        {
      ...
      const collateral = await ethers.getContractAt("MockToken", weth.address);
      ...
      expect((await pool.getAccountLiquidity(charlie.address))[1]).to.equal(
        parseEther("50")
      );

      await pool.connect(dave).depositETH({ value: parseEther("1") });
      expect((await pool.getAccountLiquidity(dave.address))[1]).to.equal(
        parseEther("10")
      );
      ...
      await synth
        .connect(charlie)
        .mint(ethers.utils.parseEther("10"), charlie.address, ZERO_ADDRESS);
      expect(
        (await pool.getAccountLiquidity(charlie.address))[2]
      ).to.be.not.equal(0);

      await synth
        .connect(dave)
        .mint(ethers.utils.parseEther("20"), dave.address, ZERO_ADDRESS);
      expect((await pool.getAccountLiquidity(dave.address))[2]).to.be.not.equal(
        0
      );
      ...
      const daveRewards = await syntheX.callStatic.getRewardsAccrued(
        [esSYX.address],
        dave.address,
        [pool.address]
      );
      ...
      await syntheX
        .connect(alice)
        .claimReward([esSYX.address], charlie.address, [pool.address]);
      await syntheX
        .connect(alice)
        .claimReward([esSYX.address], dave.address, [pool.address]);
    ...
    it("Should claim rewards with 2 pools (collecting rewards from a pool they don't belong to)", async () => {
      ...
      const pool1 = (await upgrades.deployProxy(Pool2.connect(bob), [
        "Test1",
        "TST1",
        syntheX.address,
        weth.address
      ])) as Pool;
      const pool2 = (await upgrades.deployProxy(Pool2.connect(bob), [
        "Test2",
        "TST2",
        syntheX.address,
        weth.address
      ...
      await rewardToken1
        .connect(alice)
        .mint(syntheX.address, parseEther("10000000000"));
      await rewardToken2
        .connect(alice)
        .mint(syntheX.address, parseEther("10000000000"));
      await rewardToken3
      ...
      await syntheX
        .connect(bob)
        .setPoolSpeed(rewardToken1.address, pool1.address, speed, addToList);
      await syntheX
        .connect(bob)
        .setPoolSpeed(rewardToken2.address, pool1.address, speed, addToList);
      await syntheX
        .connect(bob)
        .setPoolSpeed(rewardToken3.address, pool2.address, speed, addToList);
      ...
      const ERC20X = await ethers.getContractFactory("ERC20X");
      const synth1 = await upgrades.deployProxy(
        ERC20X,
        ["Ethereum", "ETH", pool1.address, syntheX.address],
        {
          unsafeAllow: ["delegatecall"],
        }
      );
      const synth2 = await upgrades.deployProxy(
        ERC20X,
        ["Ethereum", "ETH", pool2.address, syntheX.address],
        {
          unsafeAllow: ["delegatecall"],
        }
      );
      ...
      expect((await pool1.getAccountLiquidity(charlie.address))[1]).to.equal(
        parseEther("50")
      );
      expect((await pool2.getAccountLiquidity(charlie.address))[1]).to.equal(
        parseEther("50")
      );

      await pool1.connect(dave).depositETH({ value: parseEther("1") });
      await pool2.connect(dave).depositETH({ value: parseEther("1") });
      expect((await pool1.getAccountLiquidity(dave.address))[1]).to.equal(
        parseEther("10")
      );
      expect((await pool2.getAccountLiquidity(dave.address))[1]).to.equal(
        parseEther("10")
      );
      ...
      await synth2
        ...
        .mint(ethers.utils.parseEther("10"), charlie.address, ZERO_ADDRESS);
      expect(
        (await pool1.getAccountLiquidity(charlie.address))[2]
      ).to.be.not.equal(0);
      expect(
        (await pool2.getAccountLiquidity(charlie.address))[2]
      ).to.be.not.equal(0);
      ...
      await synth2
        ...
        .mint(ethers.utils.parseEther("20"), dave.address, ZERO_ADDRESS);
      expect(
        (await pool1.getAccountLiquidity(dave.address))[2]
      ).to.be.not.equal(0);
      expect(
        (await pool2.getAccountLiquidity(dave.address))[2]
      ).to.be.not.equal(0);
      ...
      const daveRewardsAfter = await syntheX.callStatic.getRewardsAccrued(
        [rewardToken3.address],
        dave.address,
        [pool1.address]
      );
      ...
      await synth1
        .connect(dave)
        .mint(ethers.utils.parseEther("10"), dave.address, ZERO_ADDRESS);
      await synth2
        .connect(dave)
        .mint(ethers.utils.parseEther("10"), dave.address, ZERO_ADDRESS);
      await synth1
      ...
      const daveRewardsAfter2 = await syntheX.callStatic.getRewardsAccrued(
        [rewardToken3.address],
        dave.address,
        [pool1.address]
      );
      ...
      await syntheX
        .connect(alice)
        .claimReward([rewardToken3.address], charlie.address, [pool2.address]);
      // check balance
      ...
      await syntheX
        .connect(alice)
        .claimReward([rewardToken3.address], dave.address, [pool1.address]);
      // check balance
      ...
      await syntheX
        .connect(alice)
        .claimReward([rewardToken3.address], dave.address, [pool2.address]);
      // check balance
